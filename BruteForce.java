public class BruteForce {
	...
	//беру начальный символ АСКИИ 32(пробел), конечный 127(тильда)
	...
	public static void loop(int index) {
		for(int i = min; i < max; i++) {
			...
			//если есть другой символ, он перебирает каждый символ для этого индекса
			if(index < string.length() - 1)
				loop(index + 1);
			System.out.println(string);
			//он продолжает работать, пока не будет найдено совпадение
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			for(int i = 0; i < string.length() - 1; i++) {
				//затем он перебирает каждый символ и продвигает индексный символ на единицу
				for(int j = min; j  < max; j++) {
				//после этого он повторяет каждый символ в следующем индексе строки 
	...
	//метод, который преобразует количество миллисекунд в более читаемое время
